...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 10 23:29:49 2024
...
def checkCollision(particles):
    booleanArr = np.empty_like(particles, dtype = bool())
...
def update(frame):
    ...
    scat.set_offsets(particles[:, :2])  # Update only the x, y coordinates
    return scat,

# Initialize and plot initialized particles and create bounds
fig, ax = plt.subplots()  # 2D plot
scat = ax.scatter(particles[:, 0], particles[:, 1], label='Particles')
ax.set_xlim(-0.6, 0.6)
ax.set_ylim(-0.6, 0.6)
